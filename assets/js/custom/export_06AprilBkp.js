...
const footerTemplate = globalFooterObj.template;
    ...
    const middleSectionsCookie = getCookie(GLOBAL_MIDDLE_SECTIONS_COOKIE);
...
const footerHTML = footerClone.html();
    ...
    headerPages.forEach(pageName => {
        ...
        // if (globalHeader) allSectionsOfPage.push(globalHeader);
            ...
            middleKey = keys.find(key =>
                key.toLowerCase() === pageName.toLowerCase() ||
                key.toLowerCase() === ("header_" + pageName).toLowerCase() ||
                key.toLowerCase() === ("footer_" + pageName).toLowerCase()
            );
...
allSectionsOfPage.forEach(section => {
    ...
    if (isAI && templatePath) {

        //  LOAD AI TEMPLATE WITH WRAPPER FIX
        $.ajax({
            url: templatePath,
            type: "GET",
            async: false,
            success: function (html) {
                ...
                if (parsed.find('.section-wrapper').length) {

                    sectionClone = parsed.find('.section-wrapper').first();

                } else if (parsed.filter('section').length) {
                ...
                error: function () {
                    console.error("AI load failed:", templatePath);

                    // FALLBACK TO ORIGINAL (FIX BLANK PREVIEW)
                    const originalSection =
                        document.getElementById(sectionId) ||
                        document.querySelector(`[id^="${sectionId}"]`);
...
middleKey = keys.find(key =>
 ...
 if (middleKey && middleSections[middleKey].length > 0) {
       middleSections[middleKey].forEach(section => {
    allSectionsOfPage.push(section);
});
    } else {
