...
if (sectionId.startsWith('header') || sectionId.startsWith('footer')) return;
...
if (isAI) {
    ...
    const index = PAGE_STATE[selectedPage].findIndex(sec => sec.id === sectionId);

    if (index !== -1) {
        PAGE_STATE[selectedPage][index].template = templatePath;
    } else {
    ...
    setCookie(GLOBAL_MIDDLE_SECTIONS_COOKIE, JSON.stringify(PAGE_STATE), 7);
    console.log("COOKIE NOW:", getCookie(GLOBAL_MIDDLE_SECTIONS_COOKIE));
}
...
if (!isChecked && (isHeader || isFooter)) {
    ...
    $('.custom-modal-body p').text(
        `Are you sure you want to deselect this ${type}? All designed pages will be lost.`
    );
...
function setGlobalVariablesInLocalStorage(selectedId) {
    if (selectedId.startsWith('header') || selectedId.startsWith('footer')) {
        return;
    }
...
if (selectedId.indexOf("header") === 0) {
    ...
    !selectedId.startsWith('header') &&
    !selectedId.startsWith('footer') &&
    middleSectionCategories.some(category => selectedId.startsWith(category))
) {
...
const container = $("#" + selectedId);
const existingSections = middleSectionsObject[selectedPage] || [];
const existing = existingSections.find(sec => sec.id === selectedId);
